...
const renderTags = (tags) => {
    ...
    return tags.reduce((html, tag) => {
        html += `<a href="/search/?tags=${tag}" class="tag">#${tag}</a>`
        return html
...
const renderPosts = (hash) => {
    ...
    let category = initalData.filter(item => {
    ...
    // const temp = posts.reduce((mo, p) => {
    //     mo += this.renderPost(p)
    //     return mo
    // }, '')
    // console.log(mo)
    // console.log(str,  document.getElementsByClassName('posts'))
...
if (window.location.hash) {
    ...
    fetch("/categories.json", { headers: { "Content-Type": "application/json; charset=utf-8" }})
        .then(res => res.json())
        .then(response => {
            initalData = response.filter(n => n);
